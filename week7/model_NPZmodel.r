run_model <- function(days=365) {
  
  library(ggplot2)
  
  # Framework
  days <- 365 * 2 # number of days of experiment run (units: days)
  # days <- 1 # number of days of experiment run (units: days)
  ...
  # Initial conditions
  Pinitial <- 4  # initial pphytoplankton concentration (units: mmolN m^-3)
  ...
  time <- seq(from = 0, to = days, length = NoSTEPS) # Makes and vector array of equally spaced numbers from zero to "days"
  N <- numeric(NoSTEPS) # makes a vector array of zeros (size: NoSTEPS rows by ONE column)
  P <- numeric(NoSTEPS) # same as above
  Z <- numeric(NoSTEPS) # same as above
  TotNit <- numeric(NoSTEPS) # same as above
  L_N <- numeric(NoSTEPS) # same as above
  L_P <- numeric(NoSTEPS) # same as above
  I  <- numeric(NoSTEPS) # same as above
  L_I <- numeric(NoSTEPS) # same as above
